...
public sealed class GttsViaHttpTts(
	...
	private async Task GenerateWithGttsHttp(str text, str lang, str outMp3Path, CT Ct){
		...
		var tmpPath = outMp3Path + ".tmp";
		try{
			if(File.Exists(tmpPath)){
				File.Delete(tmpPath);
			}

			await using var outFs = new FileStream(tmpPath, FileMode.Create, FileAccess.Write, FileShare.Read);
			...
			if(File.Exists(outMp3Path)){
				File.Delete(outMp3Path);
			}
			File.Move(tmpPath, outMp3Path);
		}
		catch{
			try{
				if(File.Exists(tmpPath)){
